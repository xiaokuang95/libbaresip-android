...
{
	return bytes == fwrite(data, 1, (size_t)bytes, (FILE*)fp) ? 0 : ferror((FILE*)fp);
}

static int mov_file_seek(void* fp, int64_t offset)
{
	return fseek64((FILE*)fp, (int32_t)offset, offset >= 0 ? SEEK_SET : SEEK_END);
}
...
static int mov_file_cache_read(void* fp, void* data, uint64_t bytes)
{
	...
	while (bytes > 0)
	{
		...
		if (file->off >= file->len)
		{
			...
			{
				if (bytes == fread(p, 1, (size_t)bytes, file->fp))
				{
					file->tell += bytes;
					return 0;
				}
...
static int mov_file_cache_write(void* fp, const void* data, uint64_t bytes)
{
	...
	{
		memcpy(file->ptr + file->off, data, (size_t)bytes);
...
static int mov_file_cache_seek(void* fp, int64_t offset)
{
	...
	if ((uint64_t)offset != file->tell)
	{
		...
		file->off = file->len = 0;
		r = fseek64(file->fp, (int32_t)offset, offset >= 0 ? SEEK_SET : SEEK_END);
